...
SolverKpAsInput::SolverKpAsInput(const ReferencePath &reference_path,
    ...
    : OsqpSolver(reference_path, vehicle_state, horizon),
      keep_control_steps_(
          std::max(static_cast<int>(1.2 / reference_interval_), 1)),
...
void SolverKpAsInput::getOptimizedPath(
...
void SolverKpAsInput::setHessianMatrix(
...
void SolverKpAsInput::setConstraintMatrix(
  ...
  const size_t collision_range_begin{vars_range_begin + 2 * horizon_ +
                                     control_horizon_};
  const size_t end_state_range_begin{collision_range_begin + 5 * horizon_};
  Eigen::MatrixXd cons = Eigen::MatrixXd::Zero(
      10 * horizon_ + control_horizon_ + 2, 4 * horizon_ + control_horizon_);
  // Set transition part.
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    cons(vars_range_begin + horizon_ + control_horizon_ + i,
  ...
  *upper_bound = Eigen::MatrixXd::Zero(10 * horizon_ + control_horizon_ + 2, 1);
  ...
  // Vars bound.
  for (size_t i = 0; i != horizon_; ++i) {
    (*lower_bound)(vars_range_begin + i) =
        -tan(FLAGS_max_steering_angle) / FLAGS_wheel_base;
    (*upper_bound)(vars_range_begin + i) =
        tan(FLAGS_max_steering_angle) / FLAGS_wheel_base;
    (*lower_bound)(vars_range_begin + horizon_ + control_horizon_ + i) = 0;
    (*upper_bound)(vars_range_begin + horizon_ + control_horizon_ + i) =
