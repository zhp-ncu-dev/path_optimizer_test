...
SolverKpAsInputConstrained::SolverKpAsInputConstrained(
    const ReferencePath &reference_path, const VehicleState &vehicle_state,
    const size_t &horizon)
    : OsqpSolver(reference_path, vehicle_state, horizon),
      keep_control_steps_(4),  // TODO: adjust this.
      control_horizon_((horizon_ + keep_control_steps_ - 2) /
                       keep_control_steps_),
...
void SolverKpAsInputConstrained::getOptimizedPath(
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    double tmp_x =
        ref_states[i].x + optimization_result(3 * i) * cos(new_angle);
    double tmp_y =
        ref_states[i].y + optimization_result(3 * i) * sin(new_angle);
    double k = optimization_result(3 * i + 2);
    if (i != 0) {
      tmp_s += sqrt(pow(tmp_x - optimized_path->back().x, 2) +
                    pow(tmp_y - optimized_path->back().y, 2));
    }
    optimized_path->emplace_back(
        tmp_x, tmp_y, angle + optimization_result(3 * i + 1), k, tmp_s);
  }
}

void SolverKpAsInputConstrained::setHessianMatrix(
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    hessian(state_size_ + control_size_ + i, state_size_ + control_size_ + i) +=
        w_collision_slack;
    hessian(state_size_ + control_size_ + horizon_ + i,
            state_size_ + control_size_ + horizon_ + i) += w_k_slack;
  }
  for (size_t i = 0; i != control_horizon_; ++i) {
    hessian(state_size_ + i, state_size_ + i) += keep_control_steps_ * w_cr;
    hessian(state_size_ + control_size_ + 2 * horizon_ + i,
            state_size_ + control_size_ + 2 * horizon_ + i) +=
        w_kp_slack * keep_control_steps_;
  }
  *matrix_h = hessian.sparseView();
}

void SolverKpAsInputConstrained::setConstraintMatrix(
  ...
  const size_t collision_range_begin{slack_range_begin + 2 * horizon_ +
                                     control_horizon_};
  const size_t end_state_range_begin{collision_range_begin + 5 * horizon_};
  Eigen::MatrixXd cons =
      Eigen::MatrixXd::Zero(12 * horizon_ + 3 * control_horizon_ + 2,
                            state_size_ + control_size_ + slack_size_);
  // Set transition part.
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    cons(slack_range_begin + horizon_ + i,
         state_size_ + control_size_ + horizon_ + i) = 1;
  }
  // kp:
  for (size_t i = 0; i != control_size_; ++i) {
    cons(kpl_range_begin + i, state_size_ + i) = 1;
    cons(kpl_range_begin + i, state_size_ + control_size_ + 2 * horizon_ + i) =
        1;
    cons(kpu_range_begin + i, state_size_ + i) = 1;
    cons(kpu_range_begin + i, state_size_ + control_size_ + 2 * horizon_ + i) =
        -1;
    cons(slack_range_begin + 2 * horizon_ + i,
         state_size_ + control_size_ + 2 * horizon_ + i) = 1;
  }

  // Set collision part.
  Eigen::Matrix<double, 3, 2> collision;
  collision << 1, FLAGS_d1, 1, FLAGS_d2,
      //        1, FLAGS_d3,
      1, FLAGS_d4;
  for (size_t i = 0; i != horizon_; ++i) {
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    cons.block(collision_range_begin + 3 * horizon_ + i, 3 * i, 1, 2) =
        collision1;
    cons(collision_range_begin + 3 * horizon_ + i,
         state_size_ + control_size_ + i) = -1;
    cons.block(collision_range_begin + 4 * horizon_ + i, 3 * i, 1, 2) =
        collision1;
    cons(collision_range_begin + 4 * horizon_ + i,
         state_size_ + control_size_ + i) = 1;
  }
  ...
  *matrix_constraints = cons.sparseView();
  ...
  // Set bounds.
  *lower_bound =
      Eigen::MatrixXd::Zero(12 * horizon_ + 3 * control_horizon_ + 2, 1);
  *upper_bound =
      Eigen::MatrixXd::Zero(12 * horizon_ + 3 * control_horizon_ + 2, 1);
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    (*upper_bound)(slack_range_begin + horizon_ + i) =  // OsqpEigen::INFTY;
        std::max(
            tan(FLAGS_max_steering_angle) / FLAGS_wheel_base - max_k_list[i],
            0.0);
  }
  for (size_t i = 0; i != control_horizon_; ++i) {
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    ud << bounds[i].c0.ub, bounds[i].c1.ub, bounds[i].c3.ub;
    ld << bounds[i].c0.lb, bounds[i].c1.lb, bounds[i].c3.lb;
    ...
    (*lower_bound)(collision_range_begin + 3 * horizon_ + i, 0) =
        -OsqpEigen::INFTY;
    (*lower_bound)(collision_range_begin + 4 * horizon_ + i, 0) = lds;
    (*upper_bound)(collision_range_begin + 4 * horizon_ + i, 0) =
        OsqpEigen::INFTY;
  }
  ...
  if (FLAGS_constraint_end_heading) {
    double end_psi =
        constraintAngle(vehicle_state_.getEndState().z - ref_states.back().z);
