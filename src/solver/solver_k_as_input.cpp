...
void SolverKAsInput::getOptimizedPath(
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    double tmp_x =
        ref_states[i].x + optimization_result(2 * i + 1) * cos(new_angle);
    double tmp_y =
        ref_states[i].y + optimization_result(2 * i + 1) * sin(new_angle);
    double k = 0;
    if (i != horizon_ - 1) {
    ...
    optimized_path->emplace_back(tmp_x, tmp_y,
                                 angle + optimization_result(2 * i), k, tmp_s);
  }
}

void SolverKAsInput::setHessianMatrix(
  ...
  Eigen::MatrixXd matrix_S =
      Eigen::MatrixXd::Identity(slack_size, slack_size) * w_e;

  // Fill Q, R and S into hessian matrix.
  for (size_t i = 0; i != horizon_; ++i) {
    hessian.block(2 * i, 2 * i, 2, 2) = matrix_Q;
  }
  hessian.block(2 * horizon_, 2 * horizon_, control_size, control_size) =
      matrix_R;
  hessian.block(3 * horizon_ - 1, 3 * horizon_ - 1, slack_size, slack_size) =
      matrix_S;
  *matrix_h = hessian.sparseView();
}

void SolverKAsInput::setDynamicMatrix(
    size_t i, Eigen::Matrix<double, 2, 2> *matrix_a,
...
void SolverKAsInput::setConstraintMatrix(
  ...
  // Set collision avoidance part 1. This part does not include the second
  // circle.
  Eigen::Matrix<double, 3, 2> collision;
  collision << FLAGS_d1, 1,
      //        FLAGS_d2, 1,
      FLAGS_d3, 1, FLAGS_d4, 1;
  for (size_t i = 0; i != horizon_; ++i) {
    cons.block(6 * horizon_ - 1 + 3 * i, 2 * i, 3, 2) = collision;
  }

  // Set collison avoidance part 2, This part contains the second circle only.
  // The purpose for this is to shrink the drivable corridor and then add a
  // slack variable on it.
  ...
  cons.block(9 * horizon_ - 1, 3 * horizon_ - 1, horizon_, horizon_) =
      -Eigen::MatrixXd::Identity(horizon_, horizon_);
  cons.block(10 * horizon_ - 1, 3 * horizon_ - 1, horizon_, horizon_) =
      Eigen::MatrixXd::Identity(horizon_, horizon_);
  // Finished.
  ...
  *upper_bound = Eigen::MatrixXd::Zero(11 * horizon_ - 1, 1);
  ...
  lower_bound->block(2 * horizon_, 0, 2 * horizon_, 1) =
      Eigen::VectorXd::Constant(2 * horizon_, -OsqpEigen::INFTY);
  upper_bound->block(2 * horizon_, 0, 2 * horizon_, 1) =
      Eigen::VectorXd::Constant(2 * horizon_, OsqpEigen::INFTY);
  ...
  if (FLAGS_constraint_end_heading) {
    ...
    if (end_psi < 70 * M_PI / 180) {
      (*lower_bound)(2 * horizon_ + 2 * horizon_ - 2) =
          end_psi - 5 * M_PI / 180;
      (*upper_bound)(2 * horizon_ + 2 * horizon_ - 2) =
          end_psi + 5 * M_PI / 180;
    }
  }
  ...
  lower_bound->block(5 * horizon_ - 1, 0, horizon_, 1) =
      Eigen::VectorXd::Constant(horizon_, 0);
  ...
  for (size_t i = 0; i != horizon_; ++i) {
    ...
    ld << bounds[i].c0.lb, bounds[i].c2.lb, bounds[i].c3.lb;
    lower_bound->block(6 * horizon_ - 1 + 3 * i, 0, 3, 1) = ld;
    upper_bound->block(6 * horizon_ - 1 + 3 * i, 0, 3, 1) = ud;
  }
  // Set collision bound part 2.
  upper_bound->block(10 * horizon_ - 1, 0, horizon_, 1) =
      Eigen::VectorXd::Constant(horizon_, OsqpEigen::INFTY);
  lower_bound->block(9 * horizon_ - 1, 0, horizon_, 1) =
      Eigen::VectorXd::Constant(horizon_, -OsqpEigen::INFTY);
  for (size_t i = 0; i != horizon_; ++i) {
