...
void CarGeometry::setCircles() {
  ...
  double large_circle_radius =
      sqrt(pow(width_, 2) + pow((length_ - width_) / 2.0, 2)) / 2;
  // rr
  circles_.emplace_back(rr_p_.x + small_circle_shift,
                        rr_p_.y + small_circle_shift, small_circle_radius);
  // rl
  circles_.emplace_back(rl_p_.x + small_circle_shift,
                        rl_p_.y - small_circle_shift, small_circle_radius);
  // fr
  circles_.emplace_back(fr_p_.x - small_circle_shift,
                        fr_p_.y + small_circle_shift, small_circle_radius);
  // fl
  circles_.emplace_back(fl_p_.x - small_circle_shift,
                        fl_p_.y - small_circle_shift, small_circle_radius);
  // fm
  circles_.emplace_back(bounding_c_.x + (length_ - width_) / 4, 0,
                        large_circle_radius);
  // rm
  circles_.emplace_back(bounding_c_.x - (length_ - width_) / 4, 0,
                        large_circle_radius);
...
std::vector<Circle> CarGeometry::getCircles(
    const PathOptimizationNS::State &pos) const {
