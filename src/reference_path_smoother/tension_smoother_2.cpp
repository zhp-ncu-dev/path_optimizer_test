...
FgEvalQPSmoothing::FgEvalQPSmoothing(const std::vector<double> &seg_x_list,
    ...
    : FgEvalReferenceSmoothing(seg_x_list, seg_y_list, seg_s_list,
                               seg_angle_list),
      seg_k_list_(seg_k_list) {}

void FgEvalQPSmoothing::operator()(
    PathOptimizationNS::FgEvalReferenceSmoothing::ADvector &fg,
  ...
  for (size_t i = 0; i != point_num - 1; ++i) {
    ...
    // cost
    fg[0] += FLAGS_tension_2_deviation_weight *
             (pow(cur_x - ref_x, 2) + pow(cur_y - ref_y, 2));
...
TensionSmoother2::TensionSmoother2(
...
bool TensionSmoother2::ipoptSmooth(
  ...
  for (size_t i = 0; i < point_num; i++) {
    ...
    if (i != point_num - 1) {
      vars_lowerbound[k_idx_begin + i] =
          -0.3;  //-tan(FLAGS_max_steering_angle) / FLAGS_wheel_base;
      vars_upperbound[k_idx_begin + i] =
          0.3;  // tan(FLAGS_max_steering_angle) / FLAGS_wheel_base;
    }
  }
  ...
  FgEvalQPSmoothing fg_eval_reference_smoothing(x_list, y_list, s_list,
                                                angle_list, k_list);
  // solve the problem
  CppAD::ipopt::solve<Dvector, FgEvalQPSmoothing>(
      options, vars, vars_lowerbound, vars_upperbound, constraints_lowerbound,
      constraints_upperbound, fg_eval_reference_smoothing, solution);
  // Check if it works
...
bool TensionSmoother2::osqpSmooth(
    const std::vector<double> &x_list, const std::vector<double> &y_list,
    const std::vector<double> &angle_list, const std::vector<double> &k_list,
    const std::vector<double> &s_list, std::vector<double> *result_x_list,
    std::vector<double> *result_y_list, std::vector<double> *result_s_list) {
  ...
  setConstraintMatrix(x_list, y_list, angle_list, k_list, s_list, &linearMatrix,
...
void TensionSmoother2::setHessianMatrix(
  ...
  for (int i = 0; i != size; ++i) {
    hessian(x_start_index + i, x_start_index + i) =
        hessian(y_start_index + i, y_start_index + i) =
            FLAGS_tension_2_deviation_weight * 2;
    if (i != size - 1)
      hessian(k_start_index + i, k_start_index + i) =
          FLAGS_tension_2_curvature_weight * 2;
  }
  // Curvature change.
  Eigen::Vector2d coeff_vec{1, -1};
  Eigen::Matrix2d coeff = coeff_vec * coeff_vec.transpose();
  for (int i = 0; i != size - 2; ++i) {
    hessian.block(k_start_index + i, k_start_index + i, 2, 2) +=
        2 * FLAGS_tension_2_curvature_rate_weight * coeff;
  }
  *matrix_h = hessian.sparseView();
}

void TensionSmoother2::setConstraintMatrix(
    const std::vector<double> &x_list, const std::vector<double> &y_list,
    const std::vector<double> &angle_list, const std::vector<double> &k_list,
  ...
  const size_t cons_theta_update_start_index =
      cons_y_update_start_index + size - 1;
  const size_t cons_x_index = cons_theta_update_start_index + size - 1;
  const size_t cons_y_index = cons_x_index + 1;

  Eigen::MatrixXd cons =
      Eigen::MatrixXd::Zero(3 * (size - 1) + 2, 4 * size - 1);
  ...
  for (int i = 0; i != size - 1; ++i) {
    ...
    cons(cons_x_update_start_index + i, x_start_index + i) =
        cons(cons_y_update_start_index + i, y_start_index + i) =
            cons(cons_theta_update_start_index + i, theta_start_index + i) = -1;
    cons(cons_x_update_start_index + i, theta_start_index + i) =
        ds * sin(angle_list[i]);
    cons(cons_y_update_start_index + i, theta_start_index + i) =
        -ds * cos(angle_list[i]);
  ...
  *matrix_constraints = cons.sparseView();
  ...
  for (int i = 0; i != size - 1; ++i) {
    ...
    (*lower_bound)(cons_x_update_start_index + i) =
        (*upper_bound)(cons_x_update_start_index + i) = ds * cos(angle_list[i]);
    (*lower_bound)(cons_y_update_start_index + i) =
        (*upper_bound)(cons_y_update_start_index + i) = ds * sin(angle_list[i]);
...
void TensionSmoother2::setGradient(const std::vector<double> &x_list,
  ...
  *gradient = Eigen::VectorXd::Constant(4 * size - 1, 0);
  for (int i = 0; i != size; ++i) {
    (*gradient)(x_start_index + i) =
        -2 * FLAGS_tension_2_deviation_weight * x_list[i];
    (*gradient)(y_start_index + i) =
        -2 * FLAGS_tension_2_deviation_weight * y_list[i];
  }
}
}  // namespace PathOptimizationNS