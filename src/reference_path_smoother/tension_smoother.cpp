...
void FgEvalReferenceSmoothing::operator()(
  ...
  for (size_t i = 1; i != point_num - 1; ++i) {
    ...
    ad pre_x =
        seg_x_list_[i - 1] + pre_offset * cos(seg_angle_list_[i - 1] + M_PI_2);
    ad pre_y =
        seg_y_list_[i - 1] + pre_offset * sin(seg_angle_list_[i - 1] + M_PI_2);
    ad current_x =
        seg_x_list_[i] + current_offset * cos(seg_angle_list_[i] + M_PI_2);
    ad current_y =
        seg_y_list_[i] + current_offset * sin(seg_angle_list_[i] + M_PI_2);
    ad next_x =
        seg_x_list_[i + 1] + next_offset * cos(seg_angle_list_[i + 1] + M_PI_2);
    ad next_y =
        seg_y_list_[i + 1] + next_offset * sin(seg_angle_list_[i + 1] + M_PI_2);
    ...
    fg[0] += FLAGS_cartesian_curvature_weight *
             (pow(next_x + pre_x - 2 * current_x, 2) +
              pow(next_y + pre_y - 2 * current_y, 2));
    if (i > 1) {
      ad pre_pre_offset = vars[i - 2];
      ad pre_pre_x = seg_x_list_[i - 2] +
                     pre_pre_offset * cos(seg_angle_list_[i - 2] + M_PI_2);
      ad pre_pre_y = seg_y_list_[i - 2] +
                     pre_pre_offset * sin(seg_angle_list_[i - 2] + M_PI_2);
...
bool TensionSmoother::ipoptSmooth(
  ...
  for (size_t i = 1; i != n_vars - 1; ++i) {
    ...
    // double clearance = grid_map_.getObstacleDistance(grid_map::Position(x,
    // y)); Adjust clearance.
    clearance = std::max(clearance, default_clearance);
    //            isEqual(clearance, 0) ? default_clearance :
    //                        clearance > 0.5 ? clearance - 0.5 : clearance;
    //                    clearance > FLAGS_circle_radius ? clearance -
    //                    FLAGS_circle_radius : clearance;
  ...
  FgEvalReferenceSmoothing fg_eval_reference_smoothing(x_list, y_list, s_list,
                                                       angle_list);
  // solve the problem
  CppAD::ipopt::solve<Dvector, FgEvalReferenceSmoothing>(
...
void TensionSmoother::setHessianMatrix(
  ...
  Eigen::Matrix3d dds_part{dds_vec * dds_vec.transpose() *
                           FLAGS_cartesian_curvature_weight};
  Eigen::Matrix<double, 4, 1> ddds_vec{-1, 3, -3, 1};
  Eigen::Matrix4d ddds_part{ddds_vec * ddds_vec.transpose() *
  ...
  for (int i = 0; i != size; ++i) {
    hessian(d_start_index + i, d_start_index + i) =
        FLAGS_cartesian_deviation_weight;
  }
  *matrix_h = hessian.sparseView();
}

void TensionSmoother::setConstraintMatrix(
    const std::vector<double> &x_list, const std::vector<double> &y_list,
    const std::vector<double> &angle_list, const std::vector<double> &k_list,
  ...
  *upper_bound = Eigen::MatrixXd::Zero(3 * size, 1);
  for (int i = 0; i != size; ++i) {
    // x, y and d
    cons(x_start_index + i, x_start_index + i) =
        cons(y_start_index + i, y_start_index + i) = 1;
  ...
  *matrix_constraints = cons.sparseView();
  ...
  for (size_t i = 1; i != size - 1; ++i) {
    ...
    //            isEqual(clearance, 0) ? default_clearance :
    //                   clearance > shrink_clearance ? clearance -
    //                   shrink_clearance : clearance;
