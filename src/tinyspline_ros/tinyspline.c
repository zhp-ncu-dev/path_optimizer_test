...
};                // namespace PathOptimizationNSstructtsBSplineImpl
 ...
 *                                                                             *
 ******************************************************************************/
void ts_internal_bspline_fill_knots(const tsBSpline *spline, tsBSplineType type,
...
void ts_internal_bspline_find_u(const tsBSpline *spline, tsReal u, size_t *k,
  ...
  *k = *s = 0;
  ...
  if (*s > order) longjmp(buf, TS_MULTIPLICITY);
  if (*k <= deg) /* u < u_min */
 ...
 *                                                                             *
 ******************************************************************************/
size_t ts_bspline_degree(const tsBSpline *spline) { return spline->pImpl->deg; }

tsError ts_bspline_set_degree(tsBSpline *spline, size_t deg) {
  if (deg >= ts_bspline_num_control_points(spline)) return TS_DEG_GE_NCTRLP;
...
tsError ts_bspline_set_knots(tsBSpline *spline, const tsReal *knots) {
  ...
  for (idx = 1; idx < ts_bspline_num_knots(spline); idx++) {
    ...
    if (ts_fequals(lst_knot, knot))
      ...
      mult = 0;
    if (mult > order) return TS_MULTIPLICITY;
    lst_knot = knot;
  }
...
tsReal ts_deboornet_knot(const tsDeBoorNet *net) { return net->pImpl->u; }

size_t ts_deboornet_index(const tsDeBoorNet *net) { return net->pImpl->k; }
 ...
 *                                                                             *
 ******************************************************************************/
void ts_bspline_default(tsBSpline *_spline_) { _spline_->pImpl = NULL; }
...
void ts_internal_bspline_new(size_t n_ctrlp, size_t dim, size_t deg,
  ...
  if (dim < 1) longjmp(buf, TS_DIM_ZERO);
  if (deg >= n_ctrlp) longjmp(buf, TS_DEG_GE_NCTRLP);

  _spline_->pImpl = (struct tsBSplineImpl *)malloc(sof_spline);
  if (!_spline_->pImpl) longjmp(buf, TS_MALLOC);
  ...
  TRY(b, e)
  ts_internal_bspline_fill_knots(_spline_, type, 0.f, 1.f, _spline_, b);
  CATCH
...
tsError ts_bspline_new(size_t n_ctrlp, size_t dim, size_t deg,
  ...
  jmp_buf buf;
  TRY(buf, err)
  ts_internal_bspline_new(n_ctrlp, dim, deg, type, _spline_, buf);
  CATCH
...
tsError ts_bspline_copy(const tsBSpline *original, tsBSpline *_copy_) {
  ...
  CATCH
  if (original != _copy_) ts_bspline_default(_copy_);
  ETRY return err;
}
...
void ts_bspline_free(tsBSpline *_spline_) {
  if (_spline_->pImpl) free(_spline_->pImpl);
  ts_bspline_default(_spline_);
}
...
void ts_internal_deboornet_new(const tsBSpline *spline,
  ...
  if (!_deBoorNet_->pImpl) longjmp(buf, TS_MALLOC);
...
void ts_deboornet_free(tsDeBoorNet *_deBoorNet_) {
  if (_deBoorNet_->pImpl) free(_deBoorNet_->pImpl);
  ts_deboornet_default(_deBoorNet_);
}

void ts_internal_deboornet_copy(const tsDeBoorNet *original, tsDeBoorNet *_copy,
                                jmp_buf buf) {
  size_t size;
  if (original == _copy) return;
  size = ts_internal_deboornet_sof_state(original);
  _copy->pImpl = malloc(size);
  if (!_copy->pImpl) longjmp(buf, TS_MALLOC);
...
tsError ts_deboornet_copy(const tsDeBoorNet *original, tsDeBoorNet *_copy_) {
  ...
  CATCH
  if (original != _copy_) ts_deboornet_default(_copy_);
  ETRY return err;
}
...
void ts_internal_bspline_thomas_algorithm(const tsReal *points, size_t n,
  ...
  /* input validation */
  if (dim == 0) longjmp(buf, TS_DIM_ZERO);
  if (n == 0) longjmp(buf, TS_DEG_GE_NCTRLP);
  ...
  if (m == NULL) longjmp(buf, TS_MALLOC);
  m[0] = 0.25f;
  for (i = 1; i < len_m; i++) m[i] = 1.f / (4 - m[i - 1]);
  ...
  /* back substitution */
  if (n > 3) ts_arr_fill(_result_ + lst, dim, 0.f);
  for (i = n - 2; i >= 1; i--) {
    for (d = 0; d < dim; d++) {
      k = i * dim + d;
      l = (i + 1) * dim + d;
  ...
  if (n > 3) memcpy(_result_ + lst, points + lst, sof_ctrlp);
...
void ts_internal_bspline_interpolate_cubic(const tsReal *points, size_t n,
  ...
  if (!thomas) longjmp(buf, TS_MALLOC);

  TRY(b, e)
  ts_internal_bspline_thomas_algorithm(points, n, dim, thomas, b);
  ts_internal_relaxed_uniform_cubic_bspline(thomas, n, dim, _spline_, b);
  ETRY

      free(thomas);
  if (e < 0) longjmp(buf, e);
}
...
tsError ts_bspline_interpolate_cubic(const tsReal *points, size_t n, size_t dim,
  ...
  jmp_buf buf;
  TRY(buf, err)
  ts_internal_bspline_interpolate_cubic(points, n, dim, _spline_, buf);
  CATCH
  ts_bspline_default(_spline_);
  ETRY return err;
}
...
void ts_internal_bspline_derive(const tsBSpline *spline,
  ...
  if (deg < 1 || num_ctrlp < 2) longjmp(buf, TS_UNDERIVABLE);
  ...
  if (spline == _derivative_) ts_bspline_free(_derivative_);
...
tsError ts_bspline_derive(const tsBSpline *spline, tsBSpline *_derivative_) {
  ...
  CATCH
  if (spline != _derivative_) ts_bspline_default(_derivative_);
  ETRY return err;
}
...
void ts_internal_bspline_fill_knots(const tsBSpline *spline, tsBSplineType type,
  ...
  if (n_knots < 2 * order) longjmp(buf, TS_DEG_GE_NCTRLP);
  if (type == TS_BEZIERS && n_knots % order != 0) longjmp(buf, TS_NUM_KNOTS);
  if (min > max || ts_fequals(min, max)) longjmp(buf, TS_KNOTS_DECR);
  ...
  if (type == TS_OPENED) {
    ...
    for (i = 1; i < n_knots - 1; i++) knots[i] = min + i * fac;
    ...
    for (i = order; i < n_knots - order; i++) knots[i] = min + (i - deg) * fac;
    ...
    for (i = order; i < n_knots - order; i += order)
      ts_arr_fill(knots + i, order, min + (i / order) * fac);
    ts_arr_fill(knots + i, order, max);
  }
}
...
tsError ts_bspline_fill_knots(const tsBSpline *spline, tsBSplineType type,
  ...
  jmp_buf buf;
  TRY(buf, err)
  ts_internal_bspline_fill_knots(spline, type, min, max, _result_, buf);
  CATCH
  if (spline != _result_) ts_bspline_default(_result_);
  ETRY return err;
}
...
void ts_internal_bspline_resize(const tsBSpline *spline, int n, int back,
  ...
  if (spline == _resized_) ts_bspline_free(_resized_);
...
tsError ts_bspline_resize(const tsBSpline *spline, int n, int back,
  ...
  CATCH
  if (spline != _resized_) ts_bspline_default(_resized_);
  ETRY return err;
}

void ts_internal_bspline_insert_knot(const tsBSpline *spline,
                                     const tsDeBoorNet *deBoorNet, size_t n,
                                     tsBSpline *_result_, jmp_buf buf) {
  const size_t deg = ts_bspline_degree(spline);
  const size_t dim = ts_bspline_dimension(spline);
  ...
  memmove(to, from,
          (num_ctrlp_result - n - (k - deg + N)) * sof_ctrlp); /* b) */
  /* copy knots */
  ...
  if (ts_deboornet_multiplicity(&net) == ts_bspline_order(spline)) {
    ...
    *k = ts_deboornet_index(&net);
    ...
    ts_internal_bspline_insert_knot(
        spline, &net, ts_deboornet_num_insertions(&net) + 1, _split_, b);
...
tsError ts_bspline_split(const tsBSpline *spline, tsReal u, tsBSpline *_split_,
  ...
  CATCH
  if (spline != _split_) ts_bspline_default(_split_);
  ETRY return err;
}
...
tsError ts_bspline_buckle(const tsBSpline *spline, tsReal b,
  ...
  CATCH
  if (spline != _buckled_) ts_bspline_default(_buckled_);
  ETRY return err;
}
...
void ts_internal_bspline_to_beziers(const tsBSpline *spline,
  ...
  if (spline == _beziers_) ts_bspline_free(_beziers_);
...
tsError ts_bspline_to_beziers(const tsBSpline *spline, tsBSpline *_beziers_) {
  ...
  CATCH
  if (spline != _beziers_) ts_bspline_default(_beziers_);
  ETRY return err;
}
