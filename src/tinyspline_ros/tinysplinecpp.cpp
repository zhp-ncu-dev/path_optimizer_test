 ...
 *                                                                             *
 ******************************************************************************/
tinyspline::DeBoorNet::DeBoorNet() { ts_deboornet_default(&net); }

tinyspline::DeBoorNet::DeBoorNet(const tinyspline::DeBoorNet &other) {
  tsError err = ts_deboornet_copy(&other.net, &net);
  if (err < 0) throw std::runtime_error(ts_enum_str(err));
}

tinyspline::DeBoorNet::~DeBoorNet() { ts_deboornet_free(&net); }
...
size_t tinyspline::DeBoorNet::index() const { return ts_deboornet_index(&net); }
...
std::vector<tinyspline::real> tinyspline::DeBoorNet::points() const {
  ...
  std::vector<tinyspline::real> vec = std::vector<tinyspline::real>(begin, end);
  delete points;
  return vec;
}
...
tsDeBoorNet *tinyspline::DeBoorNet::data() { return &net; }
 ...
 *                                                                             *
 ******************************************************************************/
tinyspline::BSpline::BSpline() { ts_bspline_default(&spline); }
...
tinyspline::BSpline::~BSpline() { ts_bspline_free(&spline); }
...
size_t tinyspline::BSpline::order() const { return ts_bspline_order(&spline); }
...
tsBSpline *tinyspline::BSpline::data() { return &spline; }
...
void tinyspline::BSpline::setControlPoints(
  ...
  if (expected != actual) {
    ...
    throw std::runtime_error("Expected size: " + std::string(expected_str) +
...
tinyspline::BSpline tinyspline::BSpline::fillKnots(tsBSplineType type,
  ...
  tsError err = ts_bspline_fill_knots(&spline, type, min, max, &bs.spline);
  if (err < 0) throw std::runtime_error(ts_enum_str(err));
  return bs;
}
...
tinyspline::BSpline tinyspline::Utils::interpolateCubic(
  ...
  if (dim == 0) throw std::runtime_error(ts_enum_str(TS_DIM_ZERO));
